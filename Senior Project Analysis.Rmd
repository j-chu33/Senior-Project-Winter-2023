...
```{r}
# Create a new variable for the comparison category
comparison_category <- ifelse(df_analysis$Category == "Developing", "Developed", df_analysis$Category)

# Fit the linear model with the new comparison category variable
mod_multi <- lm(CO2_Emissions ~ log(GDPPC) + comparison_category, data = df_analysis)
